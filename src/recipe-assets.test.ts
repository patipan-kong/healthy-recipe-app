...
describe('recipe image asset pipeline', () => {
  it('keeps a complete one-to-one manifest for all 50 recipes', () => {
    expect(recipes).toHaveLength(50)
    expect(recipeImageManifest).toHaveLength(50)
    expect(new Set(recipeImageManifestIds).size).toBe(50)
    expect(new Set(recipes.map(recipe => recipe.image)).size).toBe(50)
    expect(recipeImageManifest.map(entry => entry.id)).toEqual(recipes.map(recipe => recipe.id))
    expect(recipeImageManifest.every(entry => entry.prompt.includes(entry.nameEn) && entry.visualBrief.length > 40)).toBe(true)
  })
