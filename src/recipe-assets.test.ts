...
describe('recipe image asset pipeline', () => {
  it('keeps a complete one-to-one manifest for all 150 recipes', () => {
    expect(recipes).toHaveLength(150)
    expect(recipeImageManifest).toHaveLength(150)
    expect(new Set(recipeImageManifestIds).size).toBe(150)
    expect(new Set(recipes.map(recipe => recipe.image)).size).toBe(150)
