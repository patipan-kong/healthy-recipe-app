...
describe('recipe image asset pipeline', () => {
  it('keeps a complete one-to-one manifest for all 208 recipes', () => {
    expect(recipes).toHaveLength(208)
    expect(recipeImageManifest).toHaveLength(208)
    expect(new Set(recipeImageManifestIds).size).toBe(208)
    expect(new Set(recipes.map(recipe => recipe.image)).size).toBe(208)
  ...
  it('maps every expected local path to exactly one production WebP asset', () => {
    ...
    expect(new Set(files).size).toBe(208)
