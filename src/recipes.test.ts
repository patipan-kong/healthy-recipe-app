...
describe('recipe filtering', () => {
  it('uses AND semantics across category, nutrition, and tags', () => {
    ...
    expect(results.map(recipe => recipe.id)).toEqual(['herb-grilled-chicken', 'grilled-mackerel-bowl', 'thai-steamed-chicken-cabbage', 'japanese-cabbage-pork-steam', 'steamed-chicken-ginger-scallion', 'japanese-chicken-miso-ginger-bowl', 'greek-chicken-sweet-potato-tray', 'one-pan-chicken-vegetable-quinoa', 'chinese-steamed-chicken-cabbage-rolls'])
  })
...
describe('recipe search', () => {
  it('matches ingredient names and normalizes surrounding whitespace', () => {
    const results = searchRecipes(recipes, '  garlic  ').map(recipe => recipe.id)
    expect(results).toEqual(expect.arrayContaining(['tofu-mince-soup', 'herb-grilled-chicken', 'chicken-basil-rice-egg', 'tofu-basil-stir-fry', 'shrimp-tomato-pasta', 'thai-steamed-chicken-cabbage', 'light-mapo-tofu', 'white-bean-tomato-soup']))
  })
...
describe('recipe data validation', () => {
  it('contains exactly 208 image-ready curated recipes with valid catalog fields', () => {
  ...
  it('accepts complete Thai and English content for all 208 recipes', () => {
    expect(recipes).toHaveLength(208)
